...
export function TrendingSection({ trendingItems }: TrendingSectionProps) {
  ...
  const pages = Array.from({ length: totalPages }, (_, i) => {
    ...
    return trendingItems.slice(startIndex, endIndex);
  });
  ...
  // Reset animation state when transition completes
  useEffect(() => {
      ...
      <div className="relative group rounded-xl">
        ...
        <div className="overflow-hidden">
          <div 
            ...
            style={{
              height: '280px' // Adjust this height based on your item height
            }}
          >
            {pages.map((pageItems, pageIndex) => (
              ...
              >
                {pageItems.map((item, index) => (
                  <Link href={item.product_link} key={item.product_id || `${pageIndex}-${index}`}>
                    <div
